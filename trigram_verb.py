...
if __name__ == "__main__":
	...
	training_sents = utils_verb.read_tokens(training_file)
	test_file = args[1]
	test_sents = utils_verb.read_tokens(test_file)
    ...
    sents = utils_verb.read_tokens(training_file)
    predictions = predict_tags(sents, model)
    accuracy = utils_verb.calc_accuracy(training_sents, predictions)
    print "Accuracy in training [%s sentences]: %s" % (len(sents), accuracy)

    ## read sentences again because predict_tags(...) rewrites the tags
    sents = utils_verb.read_tokens(test_file)
    predictions = predict_tags(sents, model)
    accuracy = utils_verb.calc_accuracy(test_sents, predictions)
    print "Accuracy in training [%s sentences]: %s" % (len(sents), accuracy)
